...
public class RayTraceBlocksMechanic extends DirectionMechanic {
    ...
    private final CustomSkill onHit, onTick;
    ...
    public RayTraceBlocksMechanic(ConfigObject config) {
        ...
        onTick = config.contains("tick") ? MythicLib.plugin.getSkills().getSkillOrThrow(config.getString("tick")) : null;
        onHit = config.contains("hit_block") ? MythicLib.plugin.getSkills().getSkillOrThrow(config.getString("hit_block")) : null;
    ...
    public void cast(SkillMetadata meta, Location source, Vector dir) {
        ...
        if (result != null && onHit != null && result.getHitBlock() != null)
            onHit.cast(meta.clone(source, result.getHitPosition().toLocation(source.getWorld()), null));
    }
}