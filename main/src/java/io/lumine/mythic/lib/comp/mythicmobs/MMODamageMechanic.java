...
public class MMODamageMechanic extends SkillMechanic implements ITargetedEntitySkill {
    ...
    public boolean castAtEntity(SkillMetadata data, AbstractEntity target) {
        if (!target.isDead() && target.getBukkitEntity() instanceof LivingEntity && !data.getCaster().isUsingDamageSkill() && (!target.isLiving() || !(target.getHealth() <= 0.0D))) {
            ...
            MythicLib.plugin.getDamage().damage(attackMeta, (LivingEntity) target.getBukkitEntity());
